...
{   
    ...
    // Log-in form
    loginPrompt = new wxStaticText(formsPanel, wxID_ANY, L"Zaloguj się", wxPoint(150, 0));
    ...
    // Sign-up form
    signupPrompt = new wxStaticText(formsPanel, wxID_ANY, L"Nie masz konta? Zarejestruj się!", wxPoint(80, 140));
    ...
    signupPassword1->SetHint(L"Hasło");
    ...
    signupPassword2->SetHint(L"Potwierdź hasło");
    ...
    std::wstring Requirements;
    Requirements += L"* Login musi zawierać od " + std::to_string(MIN_CHAR_NAME) + " do " + std::to_string(MAX_CHAR_NAME) + L" znaków " 
        + L"i może składać się tylko z cyfr,\n podkreśleń, myślników lub małych i dużych liter alfabetu angielskiego.\n"
        + L"\n* Hasło musi zawierać: \n \t" 
        + "- od " + std::to_string(MIN_CHAR_PASSWORD) + " do " + std::to_string(MAX_CHAR_PASSWORD) + L" znaków \n \t"
        + L"- małe litery\n \t"
        + L"- duże litery\n \t"
        + "- cyfry\n \t"
        + "- znaki specjalne";
